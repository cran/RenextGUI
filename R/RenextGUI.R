...
RenextGUI <- function(guiToolkit = c("tcltk", "RGtk2", "WWW"),
  ...
  bindAllData <- function(h, ...) {
    ...
    re$dataList[[project]][["main"]] <- gsub(" ", "", svalue(datW[["main"]]))
    re$dataList[[project]][["effDuration"]] <- gsub(" ", "",  svalue(datW[["effDuration"]]))
    re$dataList[[project]][["rMax1"]] <- gsub(" ", "", svalue(datW[["rMax1"]]))
    re$dataList[[project]][["rMax1Dur"]] <- gsub(" ", "", svalue(datW[["rMax1Dur"]]))
    re$dataList[[project]][["rMax2"]] <- gsub(" ", "", svalue(datW[["rMax2"]]))
    re$dataList[[project]][["rMax2Dur"]] <- gsub(" ", "", svalue(datW[["rMax2Dur"]]))
  ...
  tryReadCsv <- function(h, ...) {
    ...
    ##--------------------------------------------------------------------------
    ## if file is changed (or at least refreshes), reinitialize the proj. name
    ##--------------------------------------------------------------------------
    ...
    ##========================================================================
    ## If we need read the file... that is if we do not simply change the
    ## variables columns or names
    ##========================================================================
    ...
    if (!is.null(h$action$source) &&
      ...
      if (DEBUG) cat("sep = ", sep, "header = ", header, "skip = ", skip, "dec = ", dec, "\n")
      ...
      if (is(res, "try-error")) {
        ...
        ##======================================================================
        ## Using the previous results, populate the selection lists for the date
        ## and for the numeric variable.
        ##======================================================================   
  ...
  actHTMLRes <- function(h, ...) {
    ...
    HTMLInsertGraph(GraphFileName = file.path(".", graphName),
                    Align = "left",
                    file =  myFile,
                    append = TRUE)
